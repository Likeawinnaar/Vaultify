...
export async function createSession(user: User, ip?: string): Promise<void> {
  const raw = randomId(32);
  const expires = new Date(Date.now() + sessionDays() * 86_400_000).toISOString();
  await run(
    "INSERT INTO sessions(id,user_id,token_hash,expires_at,created_at) VALUES(?,?,?,?,CURRENT_TIMESTAMP)",
    [randomId(), user.id, hashToken(raw), expires],
  );
  const jar = await cookies();
  jar.set("vaultify_session", raw, {
    httpOnly: true,
    sameSite: "lax",
  ...
  await audit("LOGIN", user.id, user.id, {}, ip || null);
}
...
export async function getCurrentUser(): Promise<User | null> {
  ...
  const row = await get<User>(
    "SELECT u.* FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token_hash=? AND julianday(s.expires_at) > julianday('now')",
    [hashToken(raw)],
  );
