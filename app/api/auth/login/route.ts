...
import { databaseConfigured, get, getSetting, run } from "@/lib/db";
...
export async function POST(request: Request) {
  ...
  if (!rateLimit(`login:${ip}`, 10)) return NextResponse.json({ error: "Too many login attempts. Try again later." }, { status: 429 });

  try {
    const data = loginSchema.parse(await request.json());
    const user = await get<User & { password_hash: string }>("SELECT * FROM users WHERE username=? COLLATE NOCASE OR email=? COLLATE NOCASE LIMIT 1", [data.identifier, data.identifier]);
